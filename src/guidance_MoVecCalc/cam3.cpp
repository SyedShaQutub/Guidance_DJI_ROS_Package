...
bool            show_images = 0;
uint8_t         verbosity = 0;
e_vbus_index	CAMERA_ID = e_vbus3;
DJI_lock        g_lock;
DJI_event       g_event;
...
int my_callback(int data_type, int data_len, char *content)
{
    ...
    if (e_image == data_type && NULL != content)
    {
		...
		if ( data->m_greyscale_image_left[CAMERA_ID] ){
			...
			left_8.header.frame_id  = "guidance/3/left";
		...
		if ( data->m_greyscale_image_right[CAMERA_ID] ){
			...
			right_8.header.frame_id  = "guidance/3/right";
    ...
    if ( e_obstacle_distance == data_type && NULL != content )
    {
		...
		g_oa.header.frame_id = "guidance/3/obstacle";
    ...
    if ( e_ultrasonic == data_type && NULL != content )
    {
		...
		g_ul.header.frame_id = "guidance/3/ultrasonic";
...
int main(int argc, char** argv)
{
    ...
    /* initialize ros */
    ros::init(argc, argv, "GuidanceNode_Camera3");
    ros::NodeHandle my_node;
    left_image_pub			= my_node.advertise<sensor_msgs::Image>("/guidance/3/left_image",1);
    right_image_pub			= my_node.advertise<sensor_msgs::Image>("/guidance/3/right_image",1);
    obstacle_distance_pub	= my_node.advertise<sensor_msgs::LaserScan>("/guidance/3/obstacle_distance",1);
    ultrasonic_pub			= my_node.advertise<sensor_msgs::LaserScan>("/guidance/3/ultrasonic",1);
