...
int my_callback(int data_type, int data_len, char *content)
{
    ...
    if (e_image == data_type && NULL != content)
    {
		...
		if ( data->m_greyscale_image_left[CAMERA_ID] ){
			...
			left_8.header.frame_id  = "guidance/4/left";
		...
		if ( data->m_greyscale_image_right[CAMERA_ID] ){
			...
			right_8.header.frame_id  = "guidance/4/right";
    ...
    if ( e_ultrasonic == data_type && NULL != content )
    {
		...
		g_ul.header.frame_id = "guidance/4/ultrasonic";
...
int main(int argc, char** argv)
{
    ...
    obstacle_distance_pub	= my_node.advertise<sensor_msgs::LaserScan>("/guidance/4/obstacle_distance",1);
    ultrasonic_pub			= my_node.advertise<sensor_msgs::LaserScan>("/guidance/4/ultrasonic",1);
